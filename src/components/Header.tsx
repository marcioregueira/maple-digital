  ...
  return <header className="fixed top-0 left-0 right-0 z-50 bg-black/90 backdrop-blur-md border-b border-border/50">
      <nav className="container mx-auto px-4 py-5">
        <div className="flex items-center justify-between">
          ...
          <a href="#inicio" className="flex items-center gap-3 group">
            <img alt="Maple Digital - Soluções Digitais" className="h-14 w-auto transition-opacity group-hover:opacity-80" src="/lovable-uploads/e449348e-b857-442f-9285-dd2875fa4a56.png" />
          ...
          <div className="hidden md:block">
            <Button className="bg-primary hover:bg-primary/90 text-background shadow-lg shadow-primary/30" size="sm" onClick={() => document.getElementById("contato")?.scrollIntoView({
            behavior: "smooth"
          })}>
              Solicitar Orçamento
            </Button>
      ...
      <div className={`md:hidden fixed inset-0 top-16 bg-black/95 backdrop-blur-sm transition-transform duration-300 ${isMenuOpen ? "translate-x-0" : "translate-x-full"}`}>
        <div className="flex flex-col gap-6 p-8">
          ...
          <a href="#portfolio" className="text-xl text-foreground hover:text-primary transition-colors" onClick={() => setIsMenuOpen(false)}>
            Portfólio
          </a>
